...
use four::{FourRow, GameAction, Player, GameResult, BOARD_WIDTH};
use minmax::{minmax, GameState};
...
fn create_complex_game() -> FourRow {
    ...
    for column in moves {
        let action = GameAction { column };
        if game.get_possible_actions().iter().any(|a| a.column == column) {
            game = game.apply_action(&action);
        }
    }
    game
...
fn bench_move_generation(c: &mut Criterion) {
    ...
    group.bench_with_input(BenchmarkId::new("empty_game", ""), &empty_game, |b, game| {
        b.iter(|| black_box(game.get_possible_actions()))
    });
    
    group.bench_with_input(BenchmarkId::new("early_game", ""), &early_game, |b, game| {
        b.iter(|| black_box(game.get_possible_actions()))
    });
    ...
    group.bench_with_input(BenchmarkId::new("complex_game", ""), &complex_game, |b, game| {
        b.iter(|| black_box(game.get_possible_actions()))
    });
...
fn bench_minimax_positions(c: &mut Criterion) {
    ...
    group.bench_with_input(BenchmarkId::new("empty_game", depth), &empty_game, |b, game| {
        b.iter(|| {
            let mut explored_states = HashMap::new();
            black_box(minmax(
                game.clone(),
    ...
    group.bench_with_input(BenchmarkId::new("early_game", depth), &early_game, |b, game| {
        b.iter(|| {
            let mut explored_states = HashMap::new();
            black_box(minmax(
                game.clone(),
...
fn bench_game_clone(c: &mut Criterion) {
    ...
    group.bench_function("clone_game", |b| {
